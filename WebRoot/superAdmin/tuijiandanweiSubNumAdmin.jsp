...
<title>My JSP 'tuijiandanweiSubNumAdmin.jsp' starting page</title>
	...
	<table width="98%" border="1" class="maintable">
		<tr>
			...
			<td width="82%" align="center" valign="top">
				<div class="subtitle">辽宁省科学技术协会-推荐单位提交名额管理</div>
				<div class="secondtable">
					...
					<table width="100%" border="1">
						<tr>
							<td width="10%">序号</td>
							<td width="40%">推荐单位</td>
							<td width="20%">当前可提交名额</td>
							<td width="20%">要修改的名额</td>
							<td width="10%">操作</td>
						</tr>
						<%
							for (int i = 0; i < allRefers.size(); i++) {
								ReferBean referBean = allRefers.get(i);
						%>
						<form action="ChangeReferQuota" method="get"
							name="ChangeReferQuota">
							<tr>
								<td><input type="hidden" name="id" id="id"
									value=<%=referBean.getId()%> /></td>
								<td><%=referBean.getId()%></td>
								<td><%=referBean.getName()%></td>
								<td><%=referBean.getQuota()%></td>
								<td><input type="text" name="num" id="num" /></td>
								<td><input name="change" type="submit" value="提交" /></td>
							</tr>
						</form>
						<%
							}

<%@page import="com.novae.eswsplatform.bean.ReferBean"%>
...
<title>My JSP 'tuijiandanweiAdmin.jsp' starting page</title>
...
//int allCount = adminService.getTotalDeclarer();
List<ReferBean> allRefers = adminService.getRefers();
 %>
	...
	<table width="98%" border="1" class="maintable">
		<tr>
			...
			<td width="82%" align="center" valign="top">
				<div class="subtitle">辽宁省科学技术协会-推荐单位管理</div>
				<div class="secondtable">
					<form action="abc" method="get" name="canFirstRtAdmin">
						<div style="float:left;width:40%">全省申报人数：1</div>
						<div style="float:right;width:40%">
							按推荐单位查找： <input type="text" name="sorttext" id="sorttext" /> <input
								type="submit" name="sortbutton" type="button" value="查找" />
						</div>
					</form>
					<form action="abc" method="get" name="accountApply">
						<table width="98%" border="1" style="float:left">
							<tr>
								<td width="25%">推荐单位</td>
								<td width="25%">已提交人数</td>
								<td width="25%">详情</td>
							</tr>
							<%
							for (int i=0 ; i<allRefers.size() ; i++) {
							 %>
							<tr>
								<td><%=allRefers.get(i).getName() %></td>
								<td>???</td>

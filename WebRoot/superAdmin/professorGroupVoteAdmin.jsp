...
<%@page import="com.novae.eswsplatform.service.CommitteeServiceImp"%>
<%@page import="com.novae.eswsplatform.service.CommitteeService"%>
...
<title>My JSP 'professorGroupVoteAdmin.jsp' starting page</title>
...
<%
	String defultSelect = null;
int select;
CommitteeService committeeService = new CommitteeServiceImp();
List<DeclarerBean> allDeclarers = new ArrayList<DeclarerBean>();
...
if (select==2) {
defultSelect = "理科二组";
allDeclarers = committeeService.getResultsInGroup(2);
}
%>
	...
	<table width="98%" border="1" class="maintable">
		<tr>
			...
			<td width="82%" align="center" valign="top">
				<div class="subtitle">辽宁省科学技术协会-专家组投票管理</div>
				<form action="abc" method="get" name="addGroup">
					<div style="float:left; padding-left:15px">
						选择学科分组： <select name="select" id="select" class="select"
							value=<%=defultSelect%> onchange="MM_jumpMenu('parent',this,0)">
							<option value="?select=1">理工一组</option>
							<option value="?select=2">理工二组</option>
							<option value="?select=3">理工三组</option>
							<option value="?select=4">农林组</option>
							<option value="?select=5">医药组</option>
							<option value="?select=6">交叉组</option>
						</select>
					</div>
					<div style="float:left;padding-left:150px">专家组人数：123</div>
					<div>
						<input name="reset" type="button" value="返回重新投票" />
					</div>
					<div class="secondtable" style="padding-left:310px">
						<table width="50%" border="1">
							<tr>
								<td width="20%">序号</td>
								<td width="50%">候选人姓名</td>
								<td width="30%">得票数量</td>
							</tr>
							<%
							if (allDeclarers==null){
							} else {
							for (int i = 0; i < allDeclarers.size(); i++) {
									DeclarerBean declarerBean = allDeclarers.get(i);
							%>
							<tr>
								<td><%=declarerBean.getId()%></td>
								<td><%=declarerBean.getName()%></td>
								<td><%=declarerBean.getPreJugdeCnt()%></td>
							</tr>
							<%}}%>
						</table>
				</form>
